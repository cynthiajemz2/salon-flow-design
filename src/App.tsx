...
function App() {
  ...
  useEffect(() => {
    ...
    // Find the view name based on the route path
    const viewEntry = Object.entries(ROUTES).find(([_, value]) => value === path);
    const view = (viewEntry ? viewEntry[0] : 'dashboard') as View;
    setCurrentView(view);
  }, [location.pathname]);

  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-amber-100 selection:text-amber-900 overflow-hidden">
      <Sidebar 
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 px-4 lg:px-8 flex items-center justify-between sticky top-0 z-30 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-200 focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-500 transition-all">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-4 lg:p-8 max-w-7xl mx-auto w-full min-h-full">
